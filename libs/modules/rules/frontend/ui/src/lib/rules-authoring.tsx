    ...
    <div className={styles.page}>
      ...
      {drafts.length === 0 ? (
        <Panel tone="muted" className={styles.statePanel}>
          ...
          <p>
            Create a new bounded XML source above, or open a rule or decoder snapshot record and
            clone its source file.
          </p>
        </Panel>
      ) : (
            ...
            <a key={draft.id} className={styles.snapshotLink} href={'/rules/drafts/' + draft.id}>
              <Panel tone="raised" className={styles.snapshotCard}>
                ...
                <div className={styles.inlineStats}>
                  <span>
                    Revision <strong>{draft.revision}</strong>
                  </span>
                  <span>
                    Tenant <strong>{draft.tenant}</strong>
                  </span>
                  {draft.validation ? (
                    <span>
                      Validation{' '}
                      <strong>
                        {draft.validation.errorCount} errors · {draft.validation.warningCount}{' '}
                        warnings
                      </strong>
                    </span>
                  ) : null}
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        ...
        description="Edit a database-backed working copy. Its source snapshot, when present, and the Wazuh manager remain untouched."
        actions={
          <StatusBadge tone={stateTone(draft.state)}>
      ...
      <div className={styles.headerActions}>
        ...
        {draft.sourceSnapshotId ? (
          <a className={styles.actionLink} href={'/rules/' + draft.sourceSnapshotId}>
            Source snapshot
          </a>
        ) : null}
      ...
      {error ? (
        <Panel tone="muted" className={styles.statePanel}>
          <strong>
            {error === 'conflict'
              ? 'The draft changed before your action completed.'
              : error === 'validation'
                ? 'The submitted draft content is invalid.'
                : error === 'not-found'
      ...
      <Panel tone="muted" className={styles.provenance}>
        ...
        <div>
          <span>SHA-256</span>
          <strong className={styles.mono}>{draft.sha256}</strong>
        </div>
        <div>
          <span>Updated by</span>
          <strong className={styles.mono}>{draft.updatedBy}</strong>
        </div>
      </Panel>
      ...
      <Panel tone="raised" className={styles.adminFormPanel}>
        <form action={updateAction} className={styles.adminForm}>
          ...
          <label className={styles.fullWidthField}>
            ...
            />
            <small>
              Maximum 1 MiB. Saving creates a new revision and invalidates prior
              validation/approval.
            </small>
          </label>
          <div className={styles.formActions}>
      ...
      <section className={styles.detailGrid}>
        <Panel tone="raised" className={styles.sectionPanel}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <h2>Deterministic parser checks</h2>
            </div>
            {draft.validation ? (
              <StatusBadge tone={draft.validation.errorCount > 0 ? 'accent' : 'neutral'}>
                {draft.validation.errorCount + ' errors'}
              </StatusBadge>
            ) : null}
          </div>
          ...
          {draft.validation ? (
            <>
              <div className={styles.inlineStats}>
                <span>
                  Rules <strong>{draft.validation.ruleCount}</strong>
                </span>
                <span>
                  Decoders <strong>{draft.validation.decoderCount}</strong>
                </span>
                <span>
                  Warnings <strong>{draft.validation.warningCount}</strong>
                </span>
                <span>
                    ...
                    <li key={issue.type + ':' + index}>
                      ...
                      <small>
                        {[
                          ...
                          issue.decoderName && 'Decoder ' + issue.decoderName,
                          issue.tenant,
                        ]
                          .filter(Boolean)
                          .join(' · ')}
          ...
          <form action={validateAction}>
            ...
            <button className={styles.actionButton} type="submit">
              Validate revision {draft.revision}
            </button>
          </form>
        </Panel>
        ...
        <Panel tone="raised" className={styles.sectionPanel}>
          ...
          <p className={styles.supportingText}>
            Approval is bound to the exact validated revision and SHA-256. Any edit returns the
            draft to draft state.
          </p>
          <form action={approveAction}>
            <input type="hidden" name="draftId" value={draft.id} />
            <input type="hidden" name="expectedRevision" value={draft.revision} />
            <button className={styles.actionButton} type="submit" disabled={!canApprove}>
              Approve validated revision
            </button>
          </form>
          {draft.approvedAt ? (
            <p className={styles.supportingText}>
              Approved by <span className={styles.mono}>{draft.approvedBy}</span> at{' '}
        ...
        <Panel tone="muted" className={styles.sectionPanel}>
          <div className={styles.sectionHeader}>
            <div>
              <p className={styles.eyebrow}>Deployment boundary</p>
              <h2>Export only</h2>
            </div>
          </div>
          <p className={styles.supportingText}>
            Mercure does not write this draft to a manager, archive directory, SSH target, Git
            repository, or Wazuh API.
          </p>
        </Panel>
      </section>
    </div>
