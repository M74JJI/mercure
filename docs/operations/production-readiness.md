...
11. production API and web images are fully reported for HIGH/CRITICAL Trivy findings and contain no fixable HIGH/CRITICAL OS/library vulnerabilities;
12. release archives have SHA-256 checksums, artifact-level and container-image CycloneDX SBOMs, and GitHub artifact attestations generated by the Release integrity workflow.
...
The repository includes hardened reference container definitions under `deploy/production` for the prepared API and web artifacts. Both images use the pinned Node.js runtime image digest and run as the unprivileged `node` user. They are packaging references, not an opinionated production orchestrator: TLS termination, secrets, network policy, health-route restrictions, resource limits, replicas, and rollout strategy remain deployment-environment responsibilities.

The Workspace gate builds both reference images after `pnpm release:prepare`, verifies that their configured runtime user is non-root, and scans the finished images for HIGH and CRITICAL OS/library vulnerabilities with SHA-pinned Trivy. CI reports all HIGH/CRITICAL findings, including distribution findings for which no vendor fix exists, and separately blocks every fixable HIGH/CRITICAL finding. This covers the Debian/Node/OpenSSL image layers and bundled runtime tooling that npm dependency auditing cannot see without turning an upstream no-fix advisory into a permanent merge deadlock.

For release-sensitive changes, Release integrity also builds both production images and generates container-image CycloneDX SBOMs in addition to the application-artifact SBOMs. The image SBOMs capture operating-system/base-image packages and are included in the checksummed release integrity bundle.

The API reference image explicitly installs the Debian OpenSSL/CA trust packages required by Prisma/runtime TLS dependencies. Its production install omits optional dependency closures, and both reference images remove npm/package-manager cache material that is not needed after the runtime artifact is assembled, reducing the final attack surface before image scanning. The base Node image itself remains digest-pinned; OS security packages are refreshed from the image's configured Debian repositories at image-build time.
...
docker build --file deploy/production/api.Dockerfile --tag mercure-api:release .
docker build --file deploy/production/web.Dockerfile --tag mercure-web:release .
```
